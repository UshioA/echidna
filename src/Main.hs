...
main = withUtf8 $ withCP65001 $ do
  ...
  unless cfg.solConf.quiet $
    forM_ ks $
      hPutStrLn stderr . ("Warning: unused option: " ++) . Aeson.Key.toString
...
data Options = Options
  { cliFilePath :: NE.NonEmpty FilePath,
    ...
    cliStateMachineJson :: Maybe String
  }

optsParser :: ParserInfo Options
optsParser =
  info (helper <*> versionOption <*> options) $
    fullDesc
      <> progDesc "EVM property-based testing framework"
      <> header "Echidna"

bool :: ReadM Bool
bool = maybeReader (f . map toLower)
  where
    f "true" = Just True
    f "false" = Just False
    f _ = Nothing
...
overrideConfig config Options {..} = do
  ...
  pure $
    config
      { solConf = overrideSolConf config.solConf,
  ...
  where
    ...
    overrideFormat cfg =
      case maybe cfg.uiConf.operationMode NonInteractive cliOutputFormat of
        ...
        nonInteractive ->
          cfg
            { uiConf = cfg.uiConf {operationMode = nonInteractive},
              solConf = cfg.solConf {quiet = True}
            }

    overrideCampaignConf campaignConf =
      campaignConf
