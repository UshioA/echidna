...
import Data.Time (LocalTime, NominalDiffTime, defaultTimeLocale, diffLocalTime, formatTime)
  ...
  where
    mainbox inner underneath = do
      ...
      pure $
        hCenter . hLimit 160 $
          joinBorders $
            borderWithLabel echidnaTitle $
              summaryWidget env uiState
                <=> ( if uiState.displayTestsPane
                        then
                          hBorderWithLabel
                            ( withAttr (attrName "subtitle") $
                                str $
...
summaryWidget env uiState =
  vLimit 5 $ -- limit to 5 rows
    hLimitPercent 33 leftSide
      <+> vBorder
      <+> hLimitPercent 50 middle
      <+> vBorder
      <+> rightSide
  where
    leftSide =
...
slitherWidget ::
  Bool ->
  Widget Name
slitherWidget slitherSucceeded =
  if slitherSucceeded
...
perfWidget uiState =
  ...
  where
    ...
    totalTime =
      round $
        diffLocalTime
          (fromMaybe uiState.now uiState.timeStopped)
          uiState.timeStarted
...
fetchedDialogWidget uiState =
  B.renderDialog uiState.fetchedDialog $
    padLeftRight 1 $
...
optWidget Open t =
  pure
    ( withAttr (attrName "working") $
        str $
          "optimizing, max value: " ++ show t.value,
      emptyWidget
    )
