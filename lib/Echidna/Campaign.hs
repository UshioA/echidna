...
runFuzzWorker callback vm dict workerId initialCorpus testLimit = do
  let effectiveSeed = dict.defSeed + workerId
      effectiveGenDict = dict {defSeed = effectiveSeed}
      initialState =
        WorkerState
          { workerId,
  ...
  where
    ...
    shrink = updateTests $ \test -> do
      if test.workerId == Just workerId
        then
          shrinkTest vm test
        else
          pure Nothing
...
randseq deployedContracts = do
  ...
  cmut <-
    if seqLen == 1
      then seqMutatorsStateless (fromConsts mutConsts)
        ...
        case result of
          VMSuccess (ConcreteBuf buf) -> do
            ...
            case runGetOrFail (getAbi type') (LBS.fromStrict buf) of
              ...
              Right (_, _, abiValue)
                | abiValue /= AbiAddress (forceAddr cheatCode) ->
...
updateOpenTest ::
  (MonadIO m, MonadThrow m, MonadRandom m, MonadReader Env m, MonadState WorkerState m) =>
  -- | VM after applying potential reproducer
  VM Concrete RealWorld ->
