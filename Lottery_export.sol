...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function reset() public onlyAdmin {
        ...
        if (getMatchingRewardAmount(issueIndex - 1, 4) == 0) {
            uint256 amount0 = getTotalRewards(issueIndex - 1);

            // uint256 amount1 = amount0.mul(allocation[0]);

            uint256 amount1 = amount0 * (allocation[0]);

            // uint256 amount = amount1.div(100);

            uint256 amount = amount1 / (100);
    ...
    function enterDrawingPhase() public onlyAdmin {
        ...
        // require(!(winningNumbers[0]!=0), 'drawed');
    ...
    function drawing(uint256 _externalRandomNumber) public onlyAdmin {
        ...
        // require(!(winningNumbers[0]!=0), "reset?");
        ...
        // TODO: this line i cant handle before implementing function call, so just drop it. uncomment later.
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) public {
        require(!drawed(), "drawed, can not buy now");
        ...
        Verification.Assume(keyLengthForEachBuy == 11);
        //* can terminate, does not contain require, don't do.
    ...
    function multiClaim(uint256[] memory _tickets) public {
        ...
        for (uint i = 0; i < 1; i++) {
            ...
            // // TODO: revert after done function call and TEST. my life is so bad
    ...
    function setMinPrice(uint256 _price) public onlyAdmin {
        minPrice = _price;
    }

    // Set the minimum price for one ticket

    function setMaxNumber(uint8 _maxNumber) public onlyAdmin {
        maxNumber = _maxNumber;
    }
...
contract LotteryNFT is ERC721, Ownable {
    ...
    function getLotteryAmount(uint256 tokenId) public view returns (uint256) {
        return lotteryAmount[tokenId];
    }

    function getLotteryIssueIndex(
        uint256 tokenId
    ) public view returns (uint256) {
        return issueIndex[tokenId];
    }

    function claimReward(uint256 tokenId) public onlyOwner {
        claimInfo[tokenId] = true;
    }

    function multiClaimReward(uint256[] memory _tokenIds) public onlyOwner {
